...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // TODO: integrate POST /api/v1/auth/login
      ...
      // ------------------------------------------------------------------
      // EXPLANATION OF MFA/LOGIN ROUTING LOGIC:
      // When you connect the real FastAPI endpoint, handle the response here.
      // Example:
      // const res = await fetch('/api/v1/auth/login', { method: 'POST', body: JSON.stringify(apiPayload) });
      // const data = await res.json();
      //
      // if (data.requires_mfa) {
      //    // Store temporary session token if needed
      ...
      <div className="w-full max-w-[1040px] bg-white rounded-[24px] shadow-sm border border-slate-200 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="w-full md:w-3/5 p-8 sm:p-12 lg:p-14 z-10 bg-white">
          ...
          <div className="mb-8">
            ...
            <h2 className="text-sm text-slate-500">Sign in to continue your academic journey.</h2>
          </div>

          {/* Social Logins */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
            <button type="button" className="flex justify-center items-center gap-2 py-2.5 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition-colors ${errors.email ? 'border-red-500 bg-red-50/50' : 'border-slate-200'}`}
              />
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <Link href="/forgot-password" className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 transition-colors">
                  Forgot?
                </Link>
              </div>
              <div className="relative">
                <input
                  ...
                  onChange={handleChange}
                  placeholder="••••••••"
                  className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition-colors pr-10 ${errors.password ? 'border-red-500 bg-red-50/50' : 'border-slate-200'}`}
                />
                <button 
                  type="button" 
        ...
        {/* Right Column: Platform Features Preview */}
        <div className="hidden md:flex md:w-2/5 bg-slate-100 p-10 flex-col justify-center relative overflow-hidden border-l border-slate-200">
          ...
          <div className="relative z-10 w-full max-w-sm mx-auto">
            <div className="mb-8">
              ...
              <p className="text-sm text-slate-500">Access the tools designed to help you build skills and land roles.</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white p-4 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-200 flex items-start gap-4 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-900 leading-tight">AI Resume Scorer</h4>
                  <p className="text-[12px] text-slate-500 mt-1 leading-snug">Optimize your resume structure and keywords to beat the ATS.</p>
                </div>
              </div>
              ...
              <div className="bg-white p-4 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-200 flex items-start gap-4 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-900 leading-tight">Coding Roadmaps</h4>
                  <p className="text-[12px] text-slate-500 mt-1 leading-snug">Follow structured paths for DSA, Web Dev, and System Design.</p>
                </div>
              </div>
              ...
              <div className="bg-white p-4 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-200 flex items-start gap-4 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-900 leading-tight">Smart Job Matches</h4>
                  <p className="text-[12px] text-slate-500 mt-1 leading-snug">Get curated internship and job opportunities based on your skills.</p>
                </div>
              </div>
          ...
          <div className="absolute top-[5%] right-[-10%] w-[250px] h-[250px] bg-indigo-600/5 rounded-full blur-3xl pointer-events-none"></div>
          <div className="absolute bottom-[0%] left-[-10%] w-[200px] h-[200px] bg-purple-500/5 rounded-full blur-3xl pointer-events-none"></div>
        </div>
